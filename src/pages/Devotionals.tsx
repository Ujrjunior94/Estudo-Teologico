...
import { THEMATIC_DEVOTIONALS, DevotionalItem } from '../constants/devotionals';
...
export const Devotionals: React.FC<DevotionalsProps> = ({ setActiveTab, setSelectedBibleRef }) => {
  ...
  const handleGoToBible = (scripture: string) => {
    ...
    if (scripture.includes('Eclesiastes')) {
      bookId = 'ECC';
      chapter = 4;
  ...
  const handleSaveToStudies = async (devotional: DevotionalItem, contentOverride?: string) => {
    const markdownContent = contentOverride || `
# ${devotional.title}
    ...
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <div className="border-b border-slate-200 pb-4">
        <h2 className="font-display font-bold text-3xl text-slate-900 tracking-tight">
          Devocionais Temáticos Premium
        </h2>
        <p className="text-sm text-slate-500 mt-1">
          Explore reflexões bíblicas de alta maturidade espiritual e teológica. Aprofunde seus estudos gerando reflexões sob medida com a inteligência artificial do Gemini.
        </p>
      </div>
          ...
          {selectedDevotional ? (
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center shadow-sm h-full flex flex-col justify-center items-center">
              ...
              <p className="text-sm text-slate-500 max-w-sm leading-relaxed">
                Nossos 6 planos devocionais de maturidade teológica foram desenhados para responder aos principais dilemas espirituais modernos. Escolha um tema ao lado para iniciar.
              </p>
            </div>
          )}
        </div>
